...
summarise_by_location <- function(x,post_param,output="r0"){
  ...
  plot_data <- plot_data %>% 
    ...
    dplyr::summarise(m = quantile(!!sym(output),0.5, na.rm=TRUE),
...
#' @examples
#' \dontrun{
#' create_pub_latex_tables(mod)
#' create_pub_latex_tables(intervention = mod,no_intervention= mod2)
#' }
#' @export
create_pub_latex_tables <- function(...){
  ...
  result <- res_list %>% purrr::reduce(inner_join, by=c("location"))
