...
## Cumulative cases in each outbreak over time

```{r}
df <- data.frame(BC_LTHC_outbreaks_100Imputs[[100]]$case_matrix)
colnames(df) <- BC_LTHC_outbreaks_100Imputs[[100]]$Location
...
df %>% 
  pivot_longer(-1) %>% filter(value > 0) %>% group_by(name) %>% mutate(csum = cumsum(value)) %>%
